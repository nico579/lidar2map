...
const I18N = {
  fr: {
    ...
    "remote.purge.confirm":"Voulez-vous aussi purger les fichiers de cette session sur la VM ?\n\nOui : l’état, le journal et les résultats de cette session seront supprimés ; les résultats non synchronisés seront perdus.\nNon : ces fichiers sont conservés pour inspection ou reprise.\n\nLes caches partagés restent conservés dans les deux cas.",
    ...
    "tip.ombsweep":"Algorithme SVF/openness accéléré, activé par défaut. Même rayon et même formule ; beaucoup plus rapide, avec un léger aliasing possible. Désactiver pour le calcul de référence plus lent.",
  ...
  en: {
    ...
    "remote.purge.confirm":"Do you also want to purge this session's files from the VM?\n\nYes: this session's state, log, and results will be deleted; unsynced results will be lost.\nNo: those files are kept for inspection or resume.\n\nShared caches are preserved in both cases.",
    ...
    "tip.ombsweep":"Accelerated SVF/openness algorithm, enabled by default. Same radius and formula; much faster, with slight aliasing possible. Disable it for the slower reference calculation.",
...
// ── Accès distant (hôte de confiance, VPN maillé) ──────────────────────────────
function afficherRemote() {
...
// Second serveur pour un calcul en parallèle (sans terminal, c'est le seul
// moyen : un relancement de l'application rejoint l'instance existante).
// L'onglet est ouvert TOUT DE SUITE, pendant le geste utilisateur : ouvert
...
function enregistrerHoteConfiance() {
  ...
  api.set_trusted_host(host)
    .then(r => {
      ...
      // une autre adresse que la boucle locale : rien à écouter en plus).
      let ecoute = '';
      if (host && r.ecoute === 'actif') ecoute = ' ' + tf('remote.listen.actif', {h: host, p: r.port});
      else if (host && r.ecoute === 'en attente') ecoute = ' ' + t('remote.listen.attente');
