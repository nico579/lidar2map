...
def pre_download(chemin):
    ...
    print(f"  DFM {chemin.name}: rebuilding from cached point cloud "
          f"({laz.name}, no re-download"
          f"{', CSF ~3 min' if DFM_GROUND == 'csf' else ''})...", flush=True)
...
def post_fetch(chemin):
    ...
    chemin.replace(laz)
    print(f"  DFM {chemin.name}: converting ~34M-pt cloud "
          f"({'CSF, ~3 min' if DFM_GROUND == 'csf' else '~20-30 s'})...",
          flush=True)
    common.las_to_dfm(laz, chemin, crs_epsg=2154,
                      resolution=RESOLUTION_M,
                      hmin=DFM_HMIN, hmax=DFM_HMAX,
